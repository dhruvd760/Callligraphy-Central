...
class DummySelector(BaseSelector):
    ...
    def select(self, candidates: Any) -> Any:
        if candidates == "crash":
            raise ValueError("Intentional selector crash")

        return {
            "selected": candidates[0] if isinstance(candidates, list) and candidates else candidates,
            "confidence": 0.99,
            "reason": "Best overall candidate"
        }

def test_base_selector_contract():
    class InvalidSelector(BaseSelector):
        pass
...
def test_selector_registry():
    ...
    assert registry.list_selectors() == ["dummy_selector"]
    
    assert registry.get("dummy_selector") is selector
    assert registry.get("missing") is None

def test_selector_executor():
    executor = SelectorExecutor()
    selector = DummySelector()
    
    # Valid
    res = executor.execute_select(["test data"], selector)
    assert res["select_result"]["confidence"] == 0.99
    assert res["select_result"]["reason"] == "Best overall candidate"
    assert res["select_result"]["selected"] == "test data"
...
def test_executor_exception_handling():
    ...
    # Error
    res_err = executor.execute_select("crash", selector)
    assert "error" in res_err
    assert "Intentional selector crash" in res_err["error"]

def test_selector_agent():
    registry = SelectorRegistry()
    registry.register(DummySelector())
    
    executor = SelectorExecutor()
    ctx = ContextManager()
    agent = SelectorAgent(registry, executor, ctx)
    
    # Valid
    req = json.dumps({"selector_name": "dummy_selector", "candidates": ["test candidate"]})
    res = agent.process(req)
    assert res["status"] == "success"
    assert res["selector"] == "dummy_selector"
    assert res["result"]["confidence"] == 0.99
    assert res["result"]["reason"] == "Best overall candidate"
    assert res["result"]["selected"] == "test candidate"
...
def test_missing_selector():
    ...
    res_miss = agent.process(json.dumps({"selector_name": "missing", "candidates": ["c"]}))
    assert res_miss == {"error": "Selector source 'missing' not found."}

def test_missing_selector_name():
    registry = SelectorRegistry()
    executor = SelectorExecutor()
    ctx = ContextManager()
    agent = SelectorAgent(registry, executor, ctx)
    ...
    assert res_no_name == {"error": "No 'selector_name' provided in request."}
...
def test_invalid_json():
    ...
    assert res_inv == {"error": "Invalid selector request format. Expected JSON."}
...
def test_mega_integration():
    ...
    # Final assertions
    assert res_sel["selector_agent"]["status"] == "success"
    assert res_sel["selector_agent"]["result"]["confidence"] == 0.99
    assert "Best overall candidate" in res_sel["selector_agent"]["result"]["reason"]
