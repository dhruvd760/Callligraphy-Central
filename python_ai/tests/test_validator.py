...
class DummyValidator(BaseValidator):
    ...
    def validate(self, candidate: Any) -> Any:
        if candidate == "crash":
            raise ValueError("Intentional validator crash")

        return {
            "valid": True,
            "confidence": 0.99,
            "summary": f"Validated {candidate}"
        }

def test_base_validator_contract():
...
def test_validator_registry():
    ...
    assert registry.list_validators() == ["dummy_validator"]
    
    assert registry.get("dummy_validator") is validator
    assert registry.get("missing") is None

def test_validator_executor():
    executor = ValidatorExecutor()
    validator = DummyValidator()
    
    # Valid
    res = executor.execute_validation("test data", validator)
    assert res["validation"]["valid"] is True
    assert res["validation"]["confidence"] == 0.99
    assert res["validation"]["summary"] == "Validated test data"
...
def test_executor_exception_handling():
    ...
    # Error
    res_err = executor.execute_validation("crash", validator)
    assert "error" in res_err
    assert "Intentional validator crash" in res_err["error"]
...
def test_validator_agent():
    ...
    executor = ValidatorExecutor()
    ctx = ContextManager()
    agent = ValidatorAgent(registry, executor, ctx)
    
    # Valid
    req = json.dumps({"validator_name": "dummy_validator", "candidate": "test candidate"})
    res = agent.process(req)
    assert res["status"] == "success"
    assert res["validator"] == "dummy_validator"
    assert res["result"]["valid"] is True
    assert res["result"]["confidence"] == 0.99
    assert res["result"]["summary"] == "Validated test candidate"
...
def test_missing_validator():
    ...
    res_miss = agent.process(json.dumps({"validator_name": "missing", "candidate": "c"}))
    assert res_miss == {"error": "Validator source 'missing' not found."}

def test_missing_validator_name():
    registry = ValidatorRegistry()
    executor = ValidatorExecutor()
    ctx = ContextManager()
    agent = ValidatorAgent(registry, executor, ctx)
    ...
    assert res_no_name == {"error": "No 'validator_name' provided in request."}
...
def test_invalid_json():
    ...
    assert res_inv == {"error": "Invalid validator request format. Expected JSON."}
...
def test_mega_integration():
    ...
    # Verifications
    assert res_val["validator_agent"]["status"] == "success"
    assert res_val["validator_agent"]["result"]["valid"] is True
    assert res_val["validator_agent"]["result"]["confidence"] == 0.99
