...
class DummyScorer(BaseScorer):
    ...
    def score(self, candidate: Any) -> Any:
        if candidate == "crash":
            raise ValueError("Intentional scorer crash")

        return {
            "score": 9.8,
            "confidence": 0.99,
            "reason": f"Scored {candidate}"
        }

def test_base_scorer_contract():
    class InvalidScorer(BaseScorer):
        pass
...
def test_scorer_registry():
    ...
    assert registry.list_scorers() == ["dummy_scorer"]
    
    assert registry.get("dummy_scorer") is scorer
    assert registry.get("missing") is None

def test_scorer_executor():
    executor = ScorerExecutor()
    scorer = DummyScorer()
    
    # Valid
    res = executor.execute_score("test data", scorer)
    assert res["score_result"]["score"] == 9.8
    assert res["score_result"]["confidence"] == 0.99
    assert res["score_result"]["reason"] == "Scored test data"
...
def test_executor_exception_handling():
    ...
    # Error
    res_err = executor.execute_score("crash", scorer)
    assert "error" in res_err
    assert "Intentional scorer crash" in res_err["error"]

def test_scorer_agent():
    registry = ScorerRegistry()
    registry.register(DummyScorer())
    
    executor = ScorerExecutor()
    ctx = ContextManager()
    agent = ScorerAgent(registry, executor, ctx)
    
    # Valid
    req = json.dumps({"scorer_name": "dummy_scorer", "candidate": "test candidate"})
    res = agent.process(req)
    assert res["status"] == "success"
    assert res["scorer"] == "dummy_scorer"
    assert res["result"]["score"] == 9.8
    assert res["result"]["confidence"] == 0.99
    assert res["result"]["reason"] == "Scored test candidate"

def test_missing_scorer():
    registry = ScorerRegistry()
    executor = ScorerExecutor()
    ctx = ContextManager()
    agent = ScorerAgent(registry, executor, ctx)
    
    res_miss = agent.process(json.dumps({"scorer_name": "missing", "candidate": "c"}))
    assert res_miss == {"error": "Scorer source 'missing' not found."}

def test_missing_scorer_name():
    registry = ScorerRegistry()
    executor = ScorerExecutor()
    ctx = ContextManager()
    agent = ScorerAgent(registry, executor, ctx)
    ...
    assert res_no_name == {"error": "No 'scorer_name' provided in request."}
...
def test_invalid_json():
    ...
    assert res_inv == {"error": "Invalid scorer request format. Expected JSON."}
...
def test_mega_integration():
    ...
    # Final assertions
    assert res_score["scorer_agent"]["status"] == "success"
    assert res_score["scorer_agent"]["result"]["score"] == 9.8
    assert res_score["scorer_agent"]["result"]["confidence"] == 0.99
    assert "Scored" in res_score["scorer_agent"]["result"]["reason"]
