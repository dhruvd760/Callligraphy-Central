...
class DummyRanker(BaseRanker):
    ...
    def rank(self, candidates: Any) -> Any:
        if candidates == "crash":
            raise ValueError("Intentional ranker crash")

        return {
            "winner": candidates[0] if isinstance(candidates, list) and candidates else candidates,
            "rank": 1,
            "confidence": 0.99,
            "reason": "Highest score candidate"
        }

def test_base_ranker_contract():
    class InvalidRanker(BaseRanker):
        pass
    with pytest.raises(TypeError):
        InvalidRanker()

def test_ranker_registry():
    registry = RankerRegistry()
    ranker = DummyRanker()
    
    registry.register(ranker)
    assert registry.list_rankers() == ["dummy_ranker"]
    
    assert registry.get("dummy_ranker") is ranker
    assert registry.get("missing") is None

def test_ranker_executor():
    executor = RankerExecutor()
    ranker = DummyRanker()
    
    # Valid
    res = executor.execute_rank(["test data"], ranker)
    assert res["rank_result"]["rank"] == 1
    assert res["rank_result"]["confidence"] == 0.99
    assert res["rank_result"]["reason"] == "Highest score candidate"
    assert res["rank_result"]["winner"] == "test data"
...
def test_executor_exception_handling():
    ...
    # Error
    res_err = executor.execute_rank("crash", ranker)
    assert "error" in res_err
    assert "Intentional ranker crash" in res_err["error"]

def test_ranker_agent():
    registry = RankerRegistry()
    registry.register(DummyRanker())
    
    executor = RankerExecutor()
    ctx = ContextManager()
    agent = RankerAgent(registry, executor, ctx)
    
    # Valid
    req = json.dumps({"ranker_name": "dummy_ranker", "candidates": ["test candidate"]})
    res = agent.process(req)
    assert res["status"] == "success"
    assert res["ranker"] == "dummy_ranker"
    assert res["result"]["rank"] == 1
    assert res["result"]["confidence"] == 0.99
    assert res["result"]["reason"] == "Highest score candidate"
    assert res["result"]["winner"] == "test candidate"

def test_missing_ranker():
    registry = RankerRegistry()
    executor = RankerExecutor()
    ctx = ContextManager()
    agent = RankerAgent(registry, executor, ctx)
    
    res_miss = agent.process(json.dumps({"ranker_name": "missing", "candidates": ["c"]}))
    assert res_miss == {"error": "Ranker source 'missing' not found."}

def test_missing_ranker_name():
    registry = RankerRegistry()
    executor = RankerExecutor()
    ctx = ContextManager()
    agent = RankerAgent(registry, executor, ctx)
    ...
    assert res_no_name == {"error": "No 'ranker_name' provided in request."}
...
def test_invalid_json():
    ...
    assert res_inv == {"error": "Invalid ranker request format. Expected JSON."}
...
def test_mega_integration():
    ...
    # Final assertions
    assert res_rank["ranker_agent"]["status"] == "success"
    assert res_rank["ranker_agent"]["result"]["rank"] == 1
    assert res_rank["ranker_agent"]["result"]["confidence"] == 0.99
    assert "Highest score candidate" in res_rank["ranker_agent"]["result"]["reason"]
