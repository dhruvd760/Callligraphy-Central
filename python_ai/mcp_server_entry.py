...
# Ensure the module can find python_ai packages
...
# For full integration tests, we can use the AgentRegistry. 
# But the tools themselves initialize the pipeline if an orchestrator is passed.
from memory.context_manager import ContextManager
from agents.agent_registry import AgentRegistry
from agents.orchestrator import AgentOrchestrator

# Import all agents and registries...
...
def setup_orchestrator():
    ...
    # 1. Knowledge
    class LiveKnowledge(BaseKnowledge):
        @property
    ...
    know_reg.register(LiveKnowledge())
    agent_registry.register(KnowledgeAgent(know_reg, KnowledgeExecutor(), ctx))
    
    # 2. Reasoner
    class LiveReasoner(BaseReasoner):
        @property
        def reasoner_name(self) -> str: return "dummy_reasoner"
        def reason(self, request: str, memory_context: Any) -> dict: return {"selected_route": "smart_mega_route"}
    reasoner_registry = ReasonerRegistry()
    reasoner_registry.register(LiveReasoner())
    agent_registry.register(ReasoningAgent(reasoner_registry, ReasoningExecutor(ctx), ctx))
    
    # 3. Router & 4. Planner
    class LivePlanA(BasePlan):
        @property
        def plan_name(self) -> str: return "mega_plan_a"
        def build(self, request: str) -> list: return ["prompt_agent", "extractor", "tool_agent"]
    plan_registry = PlannerRegistry()
    plan_registry.register(LivePlanA())
    ...
    route_registry.register(LiveRoute())
    ...
    conv_registry.register(LiveConv())
    ...
    # 9. Reflection
    class LiveReflection(BaseReflection):
        @property
    ...
    crit_registry.register(LiveCritic())
    agent_registry.register(CriticAgent(crit_registry, CriticExecutor(), ctx))
    
    # 11. Memory Store
    class LiveMemoryStore(BaseMemoryStore):
    ...
    eval_registry.register(LiveEvaluator())
    ...
    selector_registry.register(LiveSelector())
    agent_registry.register(SelectorAgent(selector_registry, SelectorExecutor(), ctx))

    # 17. Aggregator
    from typing import Any
    class LiveAggregator(BaseAggregator):
        @property
        def aggregator_name(self) -> str: return "dummy_aggregator"
        def aggregate(self, items: Any) -> Any: return {"combined": items, "summary": "Aggregated final MCP results"}
    aggregator_registry = AggregatorRegistry()
    aggregator_registry.register(LiveAggregator())
    agent_registry.register(AggregatorAgent(aggregator_registry, AggregatorExecutor(), ctx))
    
    return AgentOrchestrator(agent_registry, ctx)


def main():
    # Setup Orchestrator for full integration chaining
    orchestrator = setup_orchestrator()

    registry = MCPToolRegistry()
    registry.register(AnalyzeCalligraphyTool(orchestrator=orchestrator))
    registry.register(GeneratePracticeSheetTool())
    registry.register(EvaluateSubmissionTool())
    registry.register(SearchStylesTool())
    registry.register(SaveSessionTool())
    registry.register(LoadSessionTool())
    
    executor = MCPToolExecutor(registry)
    server = MCPServer(executor)

    # Indefinite stdin listener loop (Stdio implementation of MCP)
